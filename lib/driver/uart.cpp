...
void pico_uart0_handler(void)
{
    if (uart_inst0)
    {
        uart_inst0->uart_irq_rx();
        uart_inst0->uart_irq_tx();
    }
...
void pico_uart1_handler(void)
{
    if (uart_inst1)
    {
        uart_inst1->uart_irq_rx();
        uart_inst1->uart_irq_tx();
    }
...
PicoUart::PicoUart
(int uart_nr, int tx_pin, int rx_pin, int speed, int stop, int tx_size, int rx_size) :
...
int PicoUart::get_baud() { return speed; }
